...
exports.registerHead = async (req, res) => {
  ...
  if (!idProofFile || !profileFile)
    return res.status(400).json({ error: "ID Proof and Photo are required" });
  ...
  try {
    ...
    await db.query(
      `INSERT INTO district_heads (full_name, email, phone_number, district, designation, department, employee_id, office_address, password, id_proof, profile_photo, is_verified) 
...
exports.loginHead = async (req, res) => {
  ...
  try {
    ...
    if (!user.rows[0].is_verified) return res.status(403).json({ error: "Account is not verified" });
